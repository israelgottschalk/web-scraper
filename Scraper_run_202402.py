...
for row_index, row in enumerate(dataframe_to_rows(combined_data, index=False, header=False), 2):
    for col_index, cell in enumerate(row, 1):
        if isinstance(cell, pd.Timestamp):
            # Change the number format to "dd-mm-yyyy"
            ws.cell(row=row_index, column=col_index).number_format = 'dd-mm-yyyy'
